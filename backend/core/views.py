...
class PostListCreateView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            # Para garantir que o 'author' seja adicionado corretamente.
...
class LoginView(APIView):
    def post(self, request):
        ...
        try:
            ...
            if user.check_password(password):  # Verificar senha
...
def register(request):
    if request.method == "POST":
        # Carregar os dados da requisição no formato JSON
        try:
            data = json.loads(
                request.body
            )  # Certifique-se de que json é carregado de maneira correta
        except json.JSONDecodeError:
            return JsonResponse(
                {"message": "Corpo da requisição não é um JSON válido!"}, status=400
            )
        ...
        if User.objects.filter(email=email).exists():
            return JsonResponse({"message": "O email já está em uso!"}, status=400)
